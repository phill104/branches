...
function external_tracker_header($meta) {
    ...
    $stats=''; //Tracker code added if user isn't an admin, DNT is disabled and/or is using the optional cookie
    if(!(($superCage->cookie->keyExists($CONFIG['cookie_name'] . '_ext_stats')) || ($superCage->server->keyExists('HTTP_DNT')) || GALLERY_ADMIN_MODE)) {
        $stats = 'yes';
    }
